    ...
    function ConnectCall(token) {
        ...
        localStream = EnxRtc.joinRoom(token, config, function (success, error) {
            ...
            if (success && success !== null) {
                ...
                if(isModerator && room.roomSettings.canvas  )
                {
                    document.querySelector("#annotate").style.display = 'block';
                }
                ...
                room.addEventListener("active-talkers-updated", function (event) {
                    ...
                    annotateStreamID = ATList.length ? ATList[0].streamId: null;
                ...
                room.addEventListener("canvas-started", function (event) {
                    ...
                    if (ua.indexOf("safari") != -1) {
                        if (ua.indexOf("chrome") > -1) {
                            localStream.updateConfiguration(ConfigSpecs, function (
                                result
                            ) {});
                        }
                    }
...
$("#toggle_Annotate").click(function (e) {
    if(presentationStarted === true){
        return false;
    }
    isAnnotate ? (isAnnotate = false) : (isAnnotate = true);
    if (isAnnotate) {
        const stream = room.remoteStreams.get(annotateStreamID);
        room.startAnnotation(stream, function (arg) {
            console.log(arg, "arg");
        });
        $("#annotate").prop("title", "Stop Annotate");

    }else{
        room.stopAnnotation(function (arg) {
            console.log(arg, "stop");
        });
        $("#annotate").prop("title", "Annotate");
    }
});

function annotateToolActionl(action, value) {
    room.annotateToolAction(action, value);
}
